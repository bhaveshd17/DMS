...
def index(request):
    ...
    package = [float(job.salary) for job in offer_data]
    ...
    for i in offer_data:
        ...
        if 0 <= salary <= 349000:
            ctc[0] += 1
        elif 350000 <= salary <= 499000:
            ctc[1] += 1
        elif 500000 <= salary <= 700000:
            ctc[2] += 1
        elif salary >= 701000:
            ctc[3] += 1
...
def ctcWise(request):
    ...
    ctc = [sum(j[1] for data in value for j in data.values()) for key, value in ctc_dict.items()]
...
def gender_ratio(request):
    ...
    for label in labelDiv:
        gender_dict[label] = {'ptotal': 0, 'grand_total': 0, 'male': 0, 'female': 0}
...
def recruiting(request):
    ...
    hired = {}
    for job in jobs:
        count = sum(1 for p in placed if p.job_id == job)
        hired[job] = count
...
def sector(request):
    ...
    for company in companys:
        count = sum(1 for offer in offer_data if offer.job_id == company)
        company_offer[company] = count

    sectorCompany = {sector: sum(1 for company in companys if sector == company.domain) for sector in labelSector}

    content = {"labelSector": labelSector, "sectorCount": sectorCount, "companys": companys,
...
def company_student(request, id):
    ...
    studentDiv = {"INFT-A": 0, "INFT-B": 0, "INFT-C": 0, "CMPN-A": 0, "CMPN-B": 0, "CMPN-C": 0, "EXTC-A": 0,
                  "EXTC-B": 0, "EXTC-C": 0, "ETRX-A": 0, "ETRX-B": 0, "ETRX-C": 0, "BIOM": 0}
    for s in students.keys():
        if s.branch == "INFT" and s.div == "A":
            studentDiv["INFT-A"] += 1
        elif s.branch == "INFT" and s.div == "B":
            studentDiv["INFT-B"] += 1
        elif s.branch == "INFT" and s.div == "C":
            studentDiv["INFT-C"] += 1
        elif s.branch == "CMPN" and s.div == "A":
            studentDiv["CMPN-A"] += 1
        elif s.branch == "CMPN" and s.div == "B":
            studentDiv["CMPN-B"] += 1
        elif s.branch == "CMPN" and s.div == "C":
            studentDiv["CMPN-C"] += 1
        elif s.branch == "EXTC" and s.div == "A":
            studentDiv["EXTC-A"] += 1
        elif s.branch == "EXTC" and s.div == "B":
            studentDiv["EXTC-B"] += 1
        elif s.branch == "EXTC" and s.div == "C":
            studentDiv["EXTC-C"] += 1
        elif s.branch == "ETRX" and s.div == "A":
            studentDiv["ETRX-A"] += 1
        elif s.branch == "ETRX" and s.div == "B":
            studentDiv["ETRX-B"] += 1
        elif s.branch == "ETRX" and s.div == "C":
            studentDiv["ETRX-C"] += 1
        elif s.branch == "BIOM":
            studentDiv["BIOM"] += 1

    labelDiv = ["INFT-A", "INFT-B", "INFT-C", "CMPN-A", "CMPN-B", "CMPN-C", "EXTC-A", "EXTC-B", "EXTC-C", "ETRX-A",
                "ETRX-B", "ETRX-C", "BIOM"]
...
def branchWise(request, branch):
    ...
    divFemale = [0, 0, 0]
    for s in students:
        if s.div == "A" and s.gender == "Male":
            divMale[0] += 1
        elif s.div == "A" and s.gender == "Female":
            divFemale[0] += 1
        elif s.div == "B" and s.gender == "Male":
            divMale[1] += 1
        elif s.div == "B" and s.gender == "Female":
            divFemale[1] += 1
        elif s.div == "C" and s.gender == "Male":
            divMale[2] += 1
        elif s.div == "C" and s.gender == "Female":
            divFemale[2] += 1
