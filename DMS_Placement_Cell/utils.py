...
def who_can_apply_text(job: Job) -> str:
    ...
    if job.ssc_percentage != "NA":
        text += f"<li class='text-left'>Minimum {job.ssc_percentage} % of 10th required </li>"
    if job.hsc_d_percentage != "NA":
        text += f"<li class='text-left'>Minimum {job.hsc_d_percentage} % of 12/diploma required</li>"
    if job.live_kt != "NA":
        text += f"<li class='text-left'>Minimum {job.live_kt} Live KT </li>"
    if job.dead_kt != "NA":
        text += f"<li class='text-left'>Minimum {job.dead_kt} Dead KT</li>"
    if job.drop != "0":
        text += f"<li class='text-left'>Minimum {job.drop} year drop</li>"
    if text == "":
        text = "No Criteria"
    return text
